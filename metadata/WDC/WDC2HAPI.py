...
def createDateDict():
    ...
    for key in list_dict:
        ...
        glob_list = glob.glob(path1 +"/"+key + '*.wdc')
        glob_list = sorted(glob_list)
        date_dict[key] = {'startDate': list_dict[key][0][-9:-5] + '-' + glob_list[0][-7:-5] + '-01', 'endDate': list_dict[key][-1][-9:-5] + '-' + glob_list[-1][-7:-5] + '-01'}
